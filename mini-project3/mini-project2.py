...
# купить столы и стулья для зала(на 3м^2 требуется один комплект, который состоит из одного стола и 6 стульев) по цене S за один стол(один стул стоит в 3 раза меньше),
...
# Каждый месяц затраты на продукты будут увеличиваться на N тыс. руб., где N - кол-во месяцев работы кофейни.
# За каждый месяц работы с Алисы будет взят фиксированный налог, который составит 3% с дохода в первый месяц работы.
# Доход в месяц составит 175% от суммы затрат на продукты за месяц.. У Алисы есть накопленные деньги, которые она хочет вложить в открытие бизнеса - 500 000 руб.
...
# Входные данные: 45000 6000 5 Выходные данные: 316501 рублей.
# Входные данные: 10000 1000 3 Выходные данные: Алисе хватит денег, которые у нее есть.
